...
var Select = function (_React$Component) {
  ...
  function Select(props) {
    ...
    var methods = ['clickOutside', 'toggleOptions', 'onKeyDown', 'onKeyUp', 'onFocus', 'onBlur', 'onSearch', 'multiOnChange'].forEach(function (fn) {
  ...
  _createClass(Select, [{
        ...
        case 38:
          {
            if (!isOpen) {
              ...
              this.setState({
                ...
                var item = _this2['option-' + _this2.state.highlighted.value].option;
                var itemTop = item.offsetTop;
                var listTop = _this2.list.scrollTop;
                if (nextIndex === options.length - 1) {
                  var scrollTo = _this2.list.scrollHeight;
                  _this2.list.scrollTop = scrollTo;
                } else if (itemTop < listTop) {
                  var _scrollTo = itemTop;
                  _this2.list.scrollTop = _scrollTo;
                }
              });
            }
            break;
        ...
        case 40:
          {
            if (!isOpen) {
              ...
              this.setState({
                ...
                var listBottom = _this2.list.offsetHeight - _this2.list.scrollTop;
                if (itemBottom > listBottom) {
                  var scrollTo = item.offsetTop - (_this2.list.offsetHeight - item.offsetHeight);
                  _this2.list.scrollTop = scrollTo;
                }
              });
            }
    ...
    value: function onBlur() {
      ...
      this.setState({ isFocused: false }, function () {
        ...
        if (!_this3.search || _this3.search && focused !== _this3.search.input && focused !== _this3.trigger.button) {
          _this3.closeOptions(false);
        }
      });
    }
  }, {
        ...
        isOpen && _react2.default.createElement(
          ...
          _react2.default.createElement(
            ...
            options.map(function (option) {
              ...
              return _react2.default.createElement(_option2.default, { key: option.value, option: option, term: _this4.state.searchTerm, onSelect: onChange, highlighted: isHighlighted, selected: isSelected, onMouseEnter: function onMouseEnter() {
                  _this4.setState({ highlighted: option });
                }, ref: function ref(node) {
                  _this4['option-' + option.value] = node;
                } });
            })
          )
        )
